...
printf("Omega1 %s\n", value);
	...
	// some elements already exist
		...
		bcopy((pageBuf + AM_sl + numElts*recSize), &lastVal, attributeLength);
		int tempval;
		bcopy(value, &tempval, AM_si);
		...
		if(tempval == lastVal){
			...
			if(single == 0){
				...
				bcopy((pageBuf + AM_sl + numElts*recSize + attributeLength), &bagPageNum, AM_si);
				bcopy((pageBuf + AM_sl + numElts*recSize + attributeLength + AM_si), &pntr, AM_si);
				printf("Mogambo2\n");
				...
				if(PF_PAGE_SIZE - lastAddressinPage < AM_sb + AM_si*2){
					...
					*bagPage = bagPageNum;
					...
					// update that value
					bcopy(&bagPageNum, pageBuf + AM_sl + numElts*recSize + attributeLength, AM_si);
					bcopy(&pntr, pageBuf + AM_sl + numElts*recSize + attributeLength + AM_si, AM_si);
					short single1 = 0;
					bcopy(&single1, pageBuf + AM_sl + numElts*recSize + attributeLength + 2*AM_si, AM_ss);
					...
					bcopy(bagPage, pageBuf + AM_sl + numElts*recSize + attributeLength, AM_si);
					bcopy(&pntr, pageBuf + AM_sl + numElts*recSize + attributeLength + AM_si, AM_si);
					short single1 = 0;
					bcopy(&single1, pageBuf + AM_sl + numElts*recSize + attributeLength + 2*AM_si, AM_ss);
					PF_UnfixPage(fd, *bagPage, TRUE);
